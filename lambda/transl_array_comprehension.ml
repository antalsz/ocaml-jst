...
type translated_clauses =
  ...
  (** The context that must be in force throughout the entire translated
      ...
      far outwards that it isn't generated by the translation.  This is used in
      the fixed-size array case to collect the lengths of the iterators being
      iterated over, which determine the size of the array. *)
  ...
  (** The binding that defines the array size ([array_size.array_size]); comes
      ...
      Invariant: [array_size_binding.id] should be the same as
      [array_size.array_size]; we store it in both places because we want to
      pass each of these fields around separately. *)
