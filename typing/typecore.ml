      ...
      | ({pv_id = x1; pv_type = t1; pv_mode = m1; _} as pv1)::rem1,
        ...
        when Ident.equal x1 x2 ->
          if x1==x2 then
            ...
            begin try
              unify_var env (newvar (Layout.any ~creation:Dummy_layout)) t1;
              unify env t1 t2
            with
            | Unify err ->
...
and build_as_type_aux ~refine ~mode (env : Env.t ref) p =
  ...
  | Tpat_record (lpl,_) ->
      ...
      let ty = newvar (Layout.any ~creation:Dummy_layout) in
...
let solve_Ppat_tuple (type a) ~refine ~alloc_mode loc env (args : a list) expected_ty =
  ...
  let ann =
    ...
    List.map2
      (fun p mode -> (p, newgenvar (Layout.value ~creation:Tuple_element),
    ...
    List.map
      (fun name ->
        ...
        let decl = new_local_type ~loc:name.loc
                     (Layout.value ~creation:Existential_type_variable) in
        let (id, new_env) =
...
let solve_Ppat_array ~refine loc env mutability expected_ty =
  ...
  let ty_elt = newgenvar (Layout.value ~creation:Array_element) in
  let expected_ty = generic_instance expected_ty in
...
let solve_Ppat_lazy  ~refine loc env expected_ty =
  let nv = newgenvar (Layout.value ~creation:Lazy_expression) in
...
let solve_Ppat_variant ~refine loc env tag no_arg expected_ty =
  ...
  let arg_type =
    if no_arg
    then []
    else [newgenvar (Layout.value ~creation:Polymorphic_variant_field)]
  in
  let fields = [tag, rf_either ~no_arg arg_type ~matched:true] in
  let make_row more =
    create_row ~fields ~closed:false ~more ~fixed:None ~name:None
  in
  let row = make_row (newgenvar (Layout.value ~creation:Row_variable)) in
  let expected_ty = generic_instance expected_ty in
  ...
  (arg_type, make_row (newvar (Layout.value ~creation:Row_variable)),
   instance expected_ty)

(* Building the or-pattern corresponding to a polymorphic variant type *)
let build_or_pat env loc lid =
  let path, decl = Env.lookup_type ~loc:lid.loc lid.txt env in
  (* XXX layouts: the use of value here looks wrong to Richard: couldn't
     there be other layouts in a polymorphic variant argument? *)
  let tyl = List.map (fun _ -> newvar (Layout.value ~creation:Type_argument))
              decl.type_params in
  let row0 =
  ...
  let row' = ref (make_row (newvar (Layout.value ~creation:Row_variable))) in
  let pats =
    List.map
      (fun (l,p) ->
...
and type_pat_aux
  ...
  | Ppat_record(lid_sp_list, closed) ->
      ...
      let expected_type, record_ty =
        ...
        | Maybe_a_record_type ->
          None, newvar (Layout.value ~creation:Boxed_record)
        | Not_a_record_type ->
...
let type_class_arg_pattern cl_num val_env met_env l spat =
  ...
  let nv = newvar (Layout.value ~creation:Class_argument) in
  let alloc_mode = simple_pat_mode Value_mode.global in
  let pat =
  ...
  if is_optional l then
    unify_pat (ref val_env) pat
      (type_option (newvar (Layout.value ~creation:Type_argument)));
...
let type_self_pattern env spat =
  ...
  reset_pattern Modules_rejected;
  let nv = newvar (Layout.value ~creation:Object) in
  let alloc_mode = simple_pat_mode Value_mode.global in
  let pat =
    ...
    | (lbl, sarg) :: rest ->
        let (mode_arg, ty_arg_mono, mode_ret, ty_res) =
          ...
          | Tvar _ ->
              ...
              let ty_arg_mono =
                newvar (Layout.value ~creation:Function_argument)
              in
              let ty_arg = newmono ty_arg_mono in
              let ty_res = newvar (Layout.value ~creation:Function_result) in
              if ret_tvar &&
...
let rec approx_type env sty =
  ...
  | Ptyp_arrow (p, ({ ptyp_desc = Ptyp_poly _ } as arg_sty), sty) ->
      ...
      if is_optional p then newvar (Layout.value ~creation:Type_argument)
      else begin
  ...
  | Ptyp_arrow (p, arg_sty, sty) ->
      ...
      let arg =
        if is_optional p
        then type_option (newvar (Layout.value ~creation:Type_argument))
        else newvar (Layout.value ~creation:Function_argument)
      in
      let ret = approx_type env sty in
      let marg = Alloc_mode.of_const arg_mode in
      let mret = Alloc_mode.newvar () in
  ...
  | Ptyp_constr (lid, ctl) ->
      ...
      then newvar (Layout.any ~creation:Dummy_layout)
      else begin
  ...
  | _ -> newvar (Layout.any ~creation:Dummy_layout)
...
and type_approx_aux env sexp in_function ty_expected =
  ...
  | Pexp_tuple l ->
      let tys = List.map
                  (fun _ -> newvar (Layout.value ~creation:Tuple_element)) l
      in
...
let check_scope_escape_let_bound_idents env value_bindings =
  List.iter
    (fun (_, ident_loc, bound_ident_type) ->
       try unify env bound_ident_type (newvar (Layout.any ~creation:Dummy_layout))
       with Unify trace ->
...
let rec type_exp ?recarg env expected_mode sexp =
  ...
  type_expect ?recarg env expected_mode sexp
    (mk_expected (newvar (Layout.any ~creation:Dummy_layout)))
...
and type_expect_
  ...
  | Pexp_let(rec_flag, spat_sexp_list, sbody) ->
      ...
      *)
      if may_contain_modules then begin
        end_def ();
        unify_exp new_env body (newvar (Layout.any ~creation:Dummy_layout));
        if rec_flag = Recursive then
        ...
        if TypeSet.mem ty seen then false else
          match get_desc ty with
            Tarrow (_l, ty_arg, ty_fun, _com) ->
              (try unify_var env
                     (newvar (Layout.any ~creation:Dummy_layout)) ty_arg
               with Unify _ -> assert false);
              ret_tvar (TypeSet.add ty seen) ty_fun
          | Tvar _ ->
              let v = newvar (Layout.any ~creation:Dummy_layout) in
  ...
  | Pexp_match(sarg, caselist) ->
      ...
      let arg =
        type_expect env arg_expected_mode sarg
          (mk_expected (newvar (Layout.of_sort ~creation:Match sort)))
      in
      end_def ();
  ...
  | Pexp_tuple sexpl ->
      ...
      let subtypes =
        List.map (fun _ -> newgenvar (Layout.value ~creation:Tuple_element))
          sexpl
      in
  ...
  | Pexp_record(lid_sexp_list, opt_sexp) ->
      ...
      let ty_record, expected_type =
        ...
        | None, None ->
          newvar (Layout.of_new_sort_var ~creation:Record_projection), None
  ...
  | Pexp_setfield(srecord, lid, snewval) ->
      ...
      let ty_record =
        ...
        then newvar (Layout.of_new_sort_var ~creation:Record_assignment)
  ...
  | Pexp_coerce(sarg, sty, sty') ->
      ...
      let (arg, ty',cty,cty') =
        ...
        | None ->
            ...
            end_def ();
            let tv = newvar (Layout.any ~creation:Dummy_layout) in
            ...
            let id, typ =
              ...
              | Self_virtual meths_ref -> begin
                  ...
                  | exception Not_found ->
                      ...
                      let ty = newvar (Layout.value ~creation:Object_field) in
  ...
  | Pexp_lazy e ->
      let ty = newgenvar (Layout.value ~creation:Lazy_expression) in
  ...
  | Pexp_newtype({txt=name}, sbody) ->
      let layout =
        match Layout.of_attributes_default ~legacy_immediate:false
                ...
                ~default:(Layout.value ~creation:Univar) sexp.pexp_attributes
        with
        | Ok l -> l
        | Error (loc, layout) ->
  ...
  | Pexp_letop{ let_ = slet; ands = sands; body = sbody } ->
      let rec loop spat_acc ty_acc sands =
        ...
        | { pbop_pat = spat; _} :: rest ->
            ...
            let ty = newvar (Layout.value ~creation:Tuple_element) in
      ...
      let spat_params, ty_params =
        (* CR layouts v5: eliminate value requirement *)
        loop slet.pbop_pat (newvar (Layout.value ~creation:Tuple_element)) sands
      in
      (* CR layouts v2: eliminate value requirement *)
      let ty_func_result = newvar (Layout.value ~creation:Function_result) in
      ...
      in
      (* CR layouts v2: eliminate value requirement *)
      let ty_result = newvar (Layout.value ~creation:Function_result) in
      let ty_andops = newvar (Layout.value ~creation:Function_argument) in
      let ty_op =
...
and type_function ?in_function loc attrs env (expected_mode : expected_mode)
  ...
  if not has_poly && not (tpoly_is_mono ty_arg) && !Clflags.principal
    ...
    let really_poly =
      try
        unify env (newmono (newvar (Layout.any ~creation:Dummy_layout))) ty_arg;
        false
      with Unify _ -> true
    in
    Btype.backtrack snap;
      ...
      let arg =
        if vars = [] then begin
          ...
          if !Clflags.principal
            ...
            let really_poly =
              try
                unify env (newmono (newvar (Layout.any ~creation:Dummy_layout)))
                  ty_arg;
                false
              with Unify _ -> true
            in
...
and type_statement ?explanation env sexp =
  ...
  and tv = newvar (Layout.any ~creation:Dummy_layout)
  in
...
(*  Format.printf "@[%i %i@ %a@]@." lev (get_current_level())
  ...
  let ty_arg' = newvar (Layout.any ~creation:Dummy_layout) in
  let unify_pats ty =
  ...
  List.iter (fun { pat_vars; _ } ->
    iter_pattern_variables_type
      (fun t -> unify_var env (newvar (Layout.any ~creation:Dummy_layout)) t)
      pat_vars
  ) half_typed_cases;
  end_def ();
  generalize ty_arg';
  ...
  if create_inner_level then begin
    ...
    unify_exp_types loc env (instance ty_res)
      (newvar (Layout.any ~creation:Dummy_layout));
  end;
  cases, partial
...
and type_let
  ...
  let nvs = List.map (fun s -> newvar (Layout.of_sort ~creation:Let_binding s))
              sorts in
...
and type_andops env sarg sands expected_ty =
  let rec loop env let_sarg rev_sands expected_ty =
    ...
    | { pbop_op = sop; pbop_exp = sexp; pbop_loc = loc; _ } :: rest ->
        ...
        let ty_arg = newvar (Layout.value ~creation:Function_argument) in
        let ty_rest = newvar (Layout.value ~creation:Function_argument) in
        let ty_result = newvar (Layout.value ~creation:Function_result) in
        let arrow_desc = (Nolabel,Alloc_mode.global,Alloc_mode.global) in
        let ty_rest_fun =
...
and type_generic_array
  ...
  let ty = newgenvar (Layout.value ~creation:Array_element) in
...
and type_comprehension_expr
  ...
  in
  if !Clflags.principal then begin_def ();
  let element_ty = newvar (Layout.value ~creation:reason) in
  unify_exp_types
    loc
    env
...
and type_comprehension_iterator
  ...
  | In seq ->
      let item_ty = newvar (Layout.any ~creation:Dummy_layout) in
